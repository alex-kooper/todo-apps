...
class InMemoryTodoItemRepository:
    ...
    async def items(
        ...
        if list_id is None:
            ...
            if list_id not in self._list_to_item:
                raise TodoListNotFoundError(list_id)

            ret = [
                self._item_storage[item_id] for item_id in self._list_to_item[list_id]
            ]
    ...
    async def new_item(self, item: TodoItemCreate) -> TodoItem:
        if item.list_id not in self._list_to_item:
        ...
        self._list_to_item.setdefault(item.list_id, []).append(id)
        return new_item
    ...
    async def delete_item(self, id: TodoItemID) -> None:
        ...
        # Remove from list
        self._list_to_item[list_id].remove(id)
        if not self._list_to_item[list_id]:
            del self._list_to_item[list_id]

        # Remove from storage
        del self._item_storage[id]

    async def new_list(self, list_id: TodoListID) -> None:
        self._list_to_item[list_id] = []

    async def delete_list(self, list_id: TodoListID) -> None:
        if list_id not in self._list_to_item:
            return

        for item_id in self._list_to_item[list_id]:
    ...
    def _move_item(self, id: TodoItemID, list_id: TodoListID) -> None:
        if list_id not in self._list_to_item:
            raise TodoListNotFoundError(list_id)
        ...
        self._list_to_item[old_list_id].remove(id)
        if not self._list_to_item[old_list_id]:
            del self._list_to_item[old_list_id]

        self._list_to_item.setdefault(list_id, []).append(id)
